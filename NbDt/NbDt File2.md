...
3. 利用 github 或其他远程仓库协作
   ...
   我们可以用 `git clone` 从 `github.com` 克隆一个工作目录到你的虚拟机 , 他会自动设置好 `git remote` 中的信息 , 这时候我们在 `vscode` 中亦可以找到这些选项 , 轻松地将仓库拉取(替换你的工作区),推送(替换远程仓库),注意是替换 , 你的代码将会被隐藏 .
   
   而且相较于在本地进行分支合并 , 更好的实践是将分支推送到 `github` 上 , 再在 `github` 创建 `pull request` , 这样可以在线上的创建一个代表合并过程的分支 , 这样工作更容易完成 .
